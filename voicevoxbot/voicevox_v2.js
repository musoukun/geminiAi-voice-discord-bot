...
client.on("interactionCreate", async (interaction) => {
	...
	if (commandName === "vv") {
		...
		try {
			...
			if (!memories[userId]) {
				memories[userId] = new BufferMemory({
			...
			// Geminiモデルを使用して回答を生成。過去の会話履歴も渡す
			...
			player.on(AudioPlayerStatus.Idle, () => {
				...
				// 回答生成後に、現在の質問と回答をメモリに追加
				memory.saveContext(
					{ input: question },
					{ output: new AIMessage(responseText) } // outputをAIMessageでラップ
				);
			});
		} catch (error) {
			console.error("Error in command execution:", error);
